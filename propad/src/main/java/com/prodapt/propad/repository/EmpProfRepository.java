...
public interface EmpProfRepository extends JpaRepository<PropadEmpProfDetails, Integer> {
	@Query(value = "SELECT ((CASE WHEN ep_service_cert1 IS NULL THEN 1 ELSE 0 END)+ (CASE WHEN ep_service_cert2 IS NULL THEN 1 ELSE 0 END)+(CASE WHEN ep_service_cert3 IS NULL THEN 1 ELSE 0 END)+(CASE WHEN ep_payslip1 IS NULL THEN 1 ELSE 0 END)+(CASE WHEN ep_payslip2 IS NULL THEN 1 ELSE 0 END)+(CASE WHEN ep_payslip3 IS NULL THEN 1 ELSE 0 END)) AS sum_of_nulls FROM propad_emp_prof_details where ep_prof_mail=?1", nativeQuery = true)
