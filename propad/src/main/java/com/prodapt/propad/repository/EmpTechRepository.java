...
//	public static final String FIND_DOCUMENT = "select p.et_id,p.et_emp_id from propad_emp_tech_details p";
	...
	@Query(value = "SELECT ((CASE WHEN et_tech_cert1 IS NULL THEN 1 ELSE 0 END)+ (CASE WHEN et_tech_cert2 IS NULL THEN 1 ELSE 0 END)+(CASE WHEN et_tech_cert3 IS NULL THEN 1 ELSE 0 END)+(CASE WHEN et_tech_cert4 IS NULL THEN 1 ELSE 0 END)+(CASE WHEN et_tech_cert5 IS NULL THEN 1 ELSE 0 END)) AS sum_of_nulls FROM propad_emp_tech_details where et_emp_mail=?1", nativeQuery = true)
